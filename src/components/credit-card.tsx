...
export function CreditCardComponent({
  ...
  const utilizationPercentage = calculateCreditUtilization(currentBalance, creditLimit)
  ...
  return (
    <Card
      className={`relative overflow-hidden cursor-pointer transition-all hover:shadow-lg hover:scale-105 ${!isActive ? 'opacity-60' : ''}`}
      onClick={onClick}
    >
      <CardContent className="p-0">
        {/* Card Front */}
        <div className={`${getCardGradient(cardTier)} text-white p-6 relative`}>
          {/* حالة البطاقة */}
          {!isActive && (
          ...
          <div className="flex justify-between items-start mb-4">
            <div className="text-2xl">{getCardTypeIcon(cardType)}</div>
          </div>

          {/* Card Number */}
          <div className="mb-4">
            <p className="text-lg font-mono tracking-wider">
              {formatCardNumber(cardNumberLastFour)}
            </p>
          </div>
          ...
          <div className="flex justify-between items-center pt-2 border-t">
            <div className="flex items-center gap-2">
              <Badge variant="secondary" className="text-xs">
                كاش باك {formatPercentage(cashbackRate)}
              </Badge>
            </div>
            <div className="text-xs text-muted-foreground">
              تاريخ السداد: {dueDate}
            </div>
          </div>
