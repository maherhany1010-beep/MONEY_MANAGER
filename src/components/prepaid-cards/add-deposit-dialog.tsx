...
import { FeeInput, calculateFee, FeeType } from '@/components/shared/fee-input'
...
export function AddDepositDialog({ open, onOpenChange, card }: AddDepositDialogProps) {
  const { addDeposit } = usePrepaidCards()
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    switch (sourceType) {
      ...
      case 'card':
        ...
        break
      case 'prepaid':
        // For prepaid cards, we need to use the prepaid cards context
        // This is handled separately
        break
    }

    // Add deposit to card (amount only, without fees)
    addDeposit(
      card.id,
      amount,
      sourceType as any,
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          ...
          <DialogDescription>
            شحن {card.cardName} من أي مصدر متاح
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="p-3 bg-muted rounded-lg">
            ...
            <div className="flex items-center justify-between text-sm mt-1">
              ...
              <span className="font-bold text-green-600">{formatCurrency(card.balance)}</span>
            </div>
          </div>
