...
export function BalanceForecast({ accounts }: BalanceForecastProps) {
  const forecasts = useMemo(() => {
    return accounts
      .filter(a => a.isActive)
      .map((account: any) => {
        const monthlyWithdrawals = account.monthlyWithdrawals || account.totalWithdrawals || 0
        const monthlyDeposits = account.monthlyDeposits || 0
        ...
        if (netDailyFlow < 0) {
          daysUntilEmpty = Math.floor(account.balance / Math.abs(netDailyFlow))
        }

        const minBalance = account.minBalance || 0
        const balancePercentage = minBalance > 0 ? (account.balance / minBalance) * 100 : 100
    ...
    return (
      <Card>
        <CardHeader>
          ...
          <CardDescription>جميع الحسابات في وضع جيد</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="text-center py-8">
            <CheckCircle className="h-12 w-12 mx-auto mb-3 text-green-600" />
            <p className="text-sm text-muted-foreground">
              لا توجد حسابات تحتاج إلى انتباه في الوقت الحالي
            </p>
          </div>
        </CardContent>
      </Card>
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          الحسابات التي تحتاج إلى انتباه
        </CardDescription>
      </CardHeader>
      <CardContent>
            ...
            <div key={forecast.id} className="space-y-2">
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  ...
                  <p className="text-xs text-muted-foreground">{forecast.bank}</p>
                </div>
