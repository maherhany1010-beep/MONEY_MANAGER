...
-- Fixed: Drop function before recreating with new signature

-- ============================================
-- 1. DROP EXISTING FUNCTIONS (if they exist)
-- ============================================

-- Drop calculate_monthly_cashback function first
...
-- Create calculate_monthly_cashback function with correct signature
CREATE FUNCTION calculate_monthly_cashback(
    p_user_id UUID,
...
-- Only enable RLS on otp_codes if table exists
DO $$
BEGIN
...
-- Only create otp_codes index if table exists
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'otp_codes') THEN
...
-- Only analyze otp_codes if table exists
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'otp_codes') THEN
        ANALYZE otp_codes;
